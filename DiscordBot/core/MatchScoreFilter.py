...
class MatchResult:
    ...
    def target_file(self) -> str:
        # Use the match's actual date to determine the correct month folder.
        # Falling back to datetime.now() only if no match date is available.
        ref_date = self.match_date if self.match_date else datetime.now()
        current_month = ref_date.strftime("%B%y")  # e.g., "January26"
