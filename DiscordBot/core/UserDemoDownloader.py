...
class DemoDownloader:
    ...
    def check_existing_demo(self, demos_dir, match_id):
        """Check if demo already exists in compressed form (gzip or zstandard)"""
        # Check for gzipped demo
        demo_gz_path = os.path.join(demos_dir, f"{match_id}.dem.gz")
        if os.path.exists(demo_gz_path):
            try:
                # Verify it's a valid gzip file
                with gzip.open(demo_gz_path, 'rb') as f:
                    f.read(1)  # Try reading a byte to verify
                return True, demo_gz_path
            except Exception:
                # If verification fails, remove the invalid file
                os.remove(demo_gz_path)
        
        # Check for zstandard compressed demo
        demo_zst_path = os.path.join(demos_dir, f"{match_id}.dem.zst")
        if os.path.exists(demo_zst_path):
            try:
                # We can't easily verify zstandard files without the zstandard library
                # Just check if the file size is reasonable (not empty)
                if os.path.getsize(demo_zst_path) > 100:  # Arbitrary small size check
                    return True, demo_zst_path
                else:
                    os.remove(demo_zst_path)
            except Exception:
                # If verification fails, remove the invalid file
                if os.path.exists(demo_zst_path):
                    os.remove(demo_zst_path)
        
        # Default to gzip path if no existing files found
        return False, demo_gz_path
    ...
    def download_demo(self, match_id, username):
        ...
        project_dir = self.config.get('Paths', 'project_directory')
        ...
        if exists:
            file_ext = os.path.splitext(demo_path)[1]
            return False, f"Demo {match_id} already exists (found {file_ext} file)"
        ...
        try:
            ...
            temp_dir = os.path.join(self.config.get('Paths', 'project_directory'), "userdemos", "_temp", match_id)
