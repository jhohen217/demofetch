...
def get_demo_path(demo_id: str, month: str) -> Optional[str]:
    ...
    for directory in directories_to_check:
        try:
            ...
            possible_filenames = [
                f"{match_id}.dem.gz",  # Gzip compressed
                f"{match_id}.dem.zst", # Zstandard compressed
            ...
            if os.path.exists(directory):
                for filename in os.listdir(directory):
                    if match_id in filename and (filename.endswith('.dem') or filename.endswith('.dem.gz') or filename.endswith('.dem.zst')):
                        demo_path = os.path.join(directory, filename)
