...
async def prepare_parse_queue_async(month: str, config: Dict, stop_event: asyncio.Event = None, limit: int = None) -> Tuple[bool, Dict]:
    ...
    if demos_dir:
        ...
        # Function to check if a .dem file has a corresponding .dem.gz or .dem.zst file
        def has_archived_version(dem_path):
            gz_path = dem_path + '.gz'
            zst_path = dem_path + '.zst'
            return os.path.exists(gz_path) or os.path.exists(zst_path)
        
        # Scan month directory
        if os.path.exists(month_dir):
            try:
                for filename in os.listdir(month_dir):
                    if filename.endswith('.dem') and not (filename.endswith('.dem.gz') or filename.endswith('.dem.zst')):
