...
async def rebuild_parsed_file(month: str) -> Tuple[bool, str]:
    ...
    try:
        # Get configuration
        config = get_config()
        parser_output_dir = config.get('Paths', 'ParserOutput', fallback='')
        kill_collection_path = os.path.join(parser_output_dir, 'KillCollections') if parser_output_dir else ''
        if kill_collection_path: # Ensure the base KillCollections directory exists
            os.makedirs(kill_collection_path, exist_ok=True)
        textfiles_dir = config.get('Paths', 'textfiles_directory', fallback='')
        
        if not kill_collection_path or not textfiles_dir: # kill_collection_path would be empty if ParserOutput was missing
            return False, "Error: Could not find ParserOutput (for KillCollections) or textfiles directory in config"
...
async def rebuild_downloaded_file(month: str) -> Tuple[bool, str]:
    ...
    try:
        ...
        config = get_config()
        textfiles_dir = config.get('Paths', 'textfiles_directory', fallback='')
        demos_dir = config.get('Paths', 'public_demos_directory', fallback='')
        ...
        demo_files = glob.glob(os.path.join(demos_month_dir, "*.dem")) + glob.glob(os.path.join(demos_month_dir, "*.dem.gz")) + glob.glob(os.path.join(demos_month_dir, "*.dem.zst"))
        ...
        for file_path in demo_files:
            filename = os.path.basename(file_path)
            # Remove the extension (.dem, .dem.gz, or .dem.zst)
            if filename.endswith('.dem.gz'):
                demo_id = filename[:-7]  # Remove .dem.gz
            elif filename.endswith('.dem.zst'):
                demo_id = filename[:-8]  # Remove .dem.zst
...
async def rebuild_files(month: str) -> Tuple[bool, str]:
    ...
    try:
        ...
        if not textfiles_dir or not demos_dir or not kill_collection_path: # kill_collection_path would be empty if ParserOutput was missing
            return False, "Error: Could not find required directories (Textfiles, Demos, or ParserOutput for KillCollections) in config"
