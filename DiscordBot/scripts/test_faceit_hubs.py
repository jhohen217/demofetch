#!/usr/bin/env python3
"""
Script to test the response from Faceit hubs configured in config.ini.
...
class SimpleHubScraper:
    def __init__(self, hub_id, hub_name):
        ...
        config_path = os.path.join('..', '..', 'Users', 'josh', 'discordbot_pi', 'config.ini')
...
def main():
    ...
    if not hubs:
        print("No hubs defined in config.ini.")
        return
    
    print(f"Found {len(hubs)} hubs in config.ini:")
