...
# Check if config.ini exists; if not, create from example
if [ ! -f "config.ini" ] && [ -f "config.ini.example" ]; then
    echo "Config file missing. Creating default config.ini from example..."
    cp config.ini.example config.ini
    echo "Please configure 'config.ini' before running the bot."
    exit 1
fi
